...
static void put_signed(char *buffer, s21_size_t *size, intmax_t number,
  ...
  opts->width = opts->width > (int)num_len ? opts->width - (int)num_len : 0;
...
static void put_string(char *buffer, s21_size_t *size, const char *string,
  ...
  opts->width = opts->width > (int)len ? opts->width - (int)len : 0;
  ...
  if (opts->precision) {
    opts->precision = opts->precision > (int)len ? (int)len : opts->precision;
  } else {
    opts->precision = len;
  }
...
int s21_vsprintf(char *buffer, const char *fmt, va_list args) {
  s21_size_t size = 0;
  while (*fmt) {
    while (*fmt != '%' && *fmt) {
      put(buffer, size++, *fmt++);
