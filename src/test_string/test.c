...
static void test_sprintf(void) {
  ...
  result = s21_sprintf(buffer, "%.2f", 12345.6789);
  expected = sprintf(test_buffer, "%.2f", 12345.6789);
  ...
  result = s21_sprintf(buffer, "%.5s", "Hello world");
  expected = sprintf(test_buffer, "%.5s", "Hello world");

  TEST_CHECK(result == expected);
  if (result == expected) {
    TEST_CHECK(strcmp(buffer, test_buffer) == 0);
  }
  result = s21_sprintf(buffer, "%.3d", 12345);
  expected = sprintf(test_buffer, "%.3d", 12345);

  TEST_CHECK(result == expected);
  if (result == expected) {
    TEST_CHECK(strcmp(buffer, test_buffer) == 0);
  }
  result = s21_sprintf(buffer, "%.5x", 0xabcdef);
  expected = sprintf(test_buffer, "%.5x", 0xabcdef);
...
static void test_sscanf(void) {
  const char *str = "10 8 2.52.,:::'''aadadadv    2.5  545";
  char buffer[100];
  int dec;
  char test_buffer[100];
  int test_dec;

  int result = s21_sscanf(str, "%s %d", buffer, &dec);
  int expected = sscanf(str, "%s %d", test_buffer, &test_dec);

  TEST_CHECK(result == expected);
  if (result == expected) {
    TEST_CHECK(strcmp(buffer, test_buffer) == 0);
    TEST_CHECK(dec == test_dec);
  }
  float d = 0;
  float test_d = 0;

  result = s21_sscanf(str, "%f %1s", &d, buffer);
  expected = sscanf(str, "%f %1s", &test_d, test_buffer);
  TEST_CHECK(result == expected);
  if (result == expected) {
    TEST_CHECK(strcmp(buffer, test_buffer) == 0);
    TEST_CHECK(d == test_d);
  ...
  result = s21_sscanf(str, "%c %1s %2d", &c, buffer, &dec);
  expected = sscanf(str, "%c %1s %2d", &test_c, test_buffer, &test_dec);
  TEST_CHECK(result == expected);
  if (result == expected) {
    TEST_CHECK(strcmp(buffer, test_buffer) == 0);
    TEST_CHECK(c == test_c);
  ...
  int hex;
  int test_hex;
  result = s21_sscanf(str, "%x", &hex);
  expected = sscanf(str, "%x", &test_hex);
  TEST_CHECK(result == expected);
  if (result == expected) {
    TEST_CHECK(strcmp(buffer, test_buffer) == 0);
    TEST_CHECK(hex == test_hex);
  }
  int oct;
  int test_oct;
  result = s21_sscanf(str, "%o %x %d", &oct, &hex, &dec);
  expected = sscanf(str, "%o %x %d", &test_oct, &test_hex, &test_dec);
  TEST_CHECK(result == expected);
  if (result == expected) {
    TEST_CHECK(strcmp(buffer, test_buffer) == 0);
    TEST_CHECK(oct == test_oct);
    TEST_CHECK(hex == test_hex);
    TEST_CHECK(dec == test_dec);
  }
}
...
TEST_LIST = {TCASE(test_s21_memchr),  TCASE(test_s21_memcmp),
             TCASE(test_s21_memset),  TCASE(test_s21_strncat),
             TCASE(test_s21_strncmp), TCASE(test_s21_strncpy),
             TCASE(test_s21_strtok),  TCASE(test_s21_strrchr),
             TCASE(test_s21_strpbrk), TCASE(test_s21_memcpy),
             TCASE(test_s21_strstr),  TCASE(test_s21_strerror),
             TCASE(test_s21_strcspn), TCASE(test_s21_strlen),
             TCASE(test_s21_strchr),  TCASE(test_to_upper),
             TCASE(test_to_lower),    TCASE(test_trim),
             TCASE(test_insert),      TCASE(test_sprintf),
             TCASE(test_sscanf),      {NULL, NULL}};
